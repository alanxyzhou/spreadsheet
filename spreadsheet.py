...
class Calculable(Expression, abc.ABC):
    """An int literal or a cell reference."""
...
class Cell:
    """A cell within the spreadsheet."""

    def __init__(self, id: str):
        """
        id: cell id, e.g. "A1", "B3"
        """
        self.id: str = id

        self.expression: Expression = Value(0)
        """The expression tree of this cell."""

        self.value: int = 0
        """The stored value of this cell."""

        self.subscribers: Set[Cell] = set()
        """Cells subscribed to this one."""

        self.viewer = None
        """The spreadsheet UI that is viewing this value."""
    ...
    def add_subscriber(self, other):
        """Add the other Cell to this Cell's list of Cells to notify."""
        self.subscribers.add(other)

    def notify(self):
        """Our dependencies have changed. Reevaluate our display value."""
        self.set_value(self.expression)
        # self.viewer.update(self.get_value())
        for s in self.subscribers:
            s.notify()

    def __str__(self) -> str:
        return f"[{self.id}:{str(self.expression)}]"
...
class Spreadsheet:
    ...
    def parse_expr(self, raw_expr: str) -> Tuple[Expression, Set[Cell]]:
        """
        Given the well-formed expression, return a parsed Expression tree
        Return: (Parsed expression, Set of Cells referenced)
        """
    ...
    def parse_alphanumeric(self, tok: str) -> Calculable:
        """Parse out a Value or Cell reference."""
        if tok[0].isnumeric():
    ...
    def subscribe_cell(self, cell: Cell, dependent: Cell):
        """Given two Cells, subscribe the former to the latter."""
        dependent.add_subscriber(cell)

    def print(self, width: int, height: int):
        """Print this sheet at the specified size. This refreshes the entire sheet."""
        rows = []
        for h in range(height):
            row = []
            for w in range(width):
                cell = f"{chr(ord('A') + w)}{str(h + 1)}"
                row.append(self.get_cell(cell))
            rows.append(row)

        # Print
        print("---")
        for row in rows:
            print("|".join(["{0: >3}".format(str(x)) for x in row]))
        print("---")

    def __str__(self) -> str:
        import json

        cells = {str(k): str(v.expression) for k, v in self.inst().cells.items()}
